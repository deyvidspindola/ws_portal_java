...
SUN_JAVA_HOME="/u01/jdk"
export SUN_JAVA_HOME

DEFAULT_SUN_JAVA_HOME="/u01/jdk"
...
LONG_DOMAIN_HOME="/u01/oracle/user_projects/domains/domwebps01_dev"
...
# Set server startup configuration, if available. Only called if starting
...
# Properties passed in for server start only(edit for your configuration!)

JAVA_STARTUP_PROPERTIES="${JAVA_STARTUP_PROPERTIES}"
export JAVA_STARTUP_PROPERTIES
...
if [ "${stopFlag}" = "false" ] ; then
	JAVA_PROPERTIES="${JAVA_PROPERTIES} ${JAVA_STARTUP_PROPERTIES}"
	export JAVA_PROPERTIES
fi
