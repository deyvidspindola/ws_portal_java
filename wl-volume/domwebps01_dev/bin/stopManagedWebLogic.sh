...
usage()
{
	...
	echo "ADMIN_URL defaults to t3://AdminContainer:7001 if not set as an environment variable or the second command-line parameter."
	...
	echo "for example:"
	echo "$1 managedserver1 t3://AdminContainer:7001 weblogic weblogic"
}
...
if [ "$1" = "" ] ; then
	if [ "${ADMIN_URL}" = "" ] ; then
		ADMIN_URL="t3://AdminContainer:7001"
		export ADMIN_URL
	fi
else
	ADMIN_URL="$1"
