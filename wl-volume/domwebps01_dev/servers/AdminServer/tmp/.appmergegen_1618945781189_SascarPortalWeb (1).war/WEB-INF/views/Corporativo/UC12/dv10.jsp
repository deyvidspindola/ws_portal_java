...
<c:catch var="helper">
	<c:import url="/RealizarPesquisaSatisfacaoAcessoExternoServlet/consultarEstruturaPesquisa?acao=1" context="/SascarPortalWeb" />
</c:catch>

<link href="${pageContext.request.contextPath}/sascar/css/css_uc/uc07.css" media="all" rel="stylesheet" type="text/css" />	
...
<c:if test="${empty mensagem }">
	<script type="text/javascript">
		...
		function submeterPesquisa(form) {					
			...
			if (validaCamposObrigatorios()) {
				...
				$.ajax({
					url: "/SascarPortalWeb/RealizarPesquisaSatisfacaoAcessoExternoServlet/submeterRespostaPesquisa?acao=2",
					data: data || {},
					dataType:"json",
		...
		function fecharPesquisa() {			
			...
			top.document.location="http://www.sascar.com.br";
		}
	...
	<div id="header">
		<div id="logo">
			<a href="#" onclick="cancelarPesquisa()">
		...
		<c:choose>
			...
			<c:otherwise>				
				<form id="pesquisaSatisfacao" action="" method="post" >	
					...
					<input type="hidden" name="cC" id="cC" value="${codigoCliente}">					
					...
					<c:choose>
						<c:when test="${not empty formulario.statuspesquisa}">
							...
							<div style="padding: 10px;">
								<input type="reset" value="<fmt:message key="label.cancelar" />" class="button" onclick="cancelarPesquisa();"/>
							</div>	
						</c:when>
						<c:otherwise>
							<!--  -->
							<ul class="formulario">
								<li class="infoGeral">
									<span class="st-titulo text5"><fmt:message key="label.pesquisaSatisfacao" /> - ${formulario.tipoPesquisa}</span>
									<c:choose>
																	...
																	<ul>
																		...
																		<li class="descricao">${counter.count}. ${pergunta.descricao }															
														                	<c:if test="${ not empty pergunta.imagem}">
															                  <br>                                     
															                  <a class="linkazulescuro" style="cursor:pointer" onclick="window.open('${caminhoImagem}${pergunta.imagem}', '_blank')" >(Visualizar Imagem)</a>
															                </c:if>
																		</li>
							...
							<div style="padding: 10px;">
								<input type="reset" value="<fmt:message key="label.responderDepois" />" class="button" onclick="cancelarPesquisa();"/>
								<input id="button" 
										class="button"
										type="button"
